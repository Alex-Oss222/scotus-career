...
2. A completed adjudicative entry is never silently rewritten. A correction the user authorizes replaces the superseded entry in place — in its record file, in its chunk’s Render Input, and in its chunk’s render — names what it corrects, and carries a lineage line (versions and commit hashes); every earlier version stays in repository history. During staged close, candidate trackers and `close/AUDIT.md` are replaceable working artifacts so the directory contains only the current versions. After Commit, candidates are deleted and one final `AUDIT.md` remains. Pass notes and the Term-Close Dossier remain historical close records. (Convention changed September 15, 2026 at the user’s direction; the Holmes, Suter/Montana, Alaska and Harris corrections predate it and remain as separate files.)
